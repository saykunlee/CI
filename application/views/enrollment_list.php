  ...
  <main>
    <div class="d-flex flex-column flex-shrink-0 p-3 bg-light" style="width: 220px;">
      ...
      <ul class="nav nav-pills flex-column mb-auto">
        ...
        <li>
          <a href="/course/enrollment" class="nav-link active">수강신청현황</a>
        </li>
        <li>
          <a href="/instructor/list" class="nav-link link-dark">교수진</a>
        </li>
      </ul>
      <hr>
      <div class="dropdown">
    ...
    <div class="container mt-3">
      <h3>수강신청현황</h3>
      <p class="lead">수강신청 리스트를 확인하세요.</p>
      <table class="table  table-hover small">
        ...
        <tbody>
          <?php foreach ($data['enrollment'] as $key => $value) { ?>
            <tr>
              <th scope="row"><?php echo $key + 1 ?></th> <!-- 강의번호 -->
              <td><?php echo $value->course_name ?></td><!-- 강의명 -->
              <td> <?php echo nice_date($value->course_time, 'H:i'); ?></td><!-- 시간 -->
              <td> <?php echo $value->course_point; ?></td><!-- 이수학점 -->
              <td> <?php echo $value->course_classno; ?></td><!-- 강의실 -->
              <td> <?php echo $value->instructor_name; ?></td><!-- 교수명 -->
              <td> <?php echo $value->enrollment_date; ?></td><!-- 수강신청일자 -->
              <td> <a href="/course/del_enrollment/<?php echo $value->enrollment_no ?> " class="btn btn-outline-danger btn-sm">수강취소</a></td><!-- 수강취소 버튼 -->
            </tr>
          <?php } ?>
        </tbody>
      </table>
